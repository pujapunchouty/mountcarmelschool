...
jQuery(document).ready(function($) {
			...
			container.css({
				...
				.click(function(e){
					...
					if ( target.is(gallery) || target.parents().add(target).is(close_hint) ) {
						...
						container.animate({scrollTop: parseInt(info.position()['top'], 10)}, 'slow');
	...
	var methods = {
		...
		loopSlides : function(reverse){
			...
			gallery.jp_carousel('selectedSlide').removeClass('selected').css({'position': 'absolute'});
			if (reverse !== true ) {
				last = slides.last();
		...
		selectSlide : function(slide, animate){
			var last = this.find('.selected').removeClass('selected'),
				slides = gallery.jp_carousel('slides').css({'position': 'absolute'}),
		...
		getComments: function( args ) {
			...
			$.ajax({
				...
				success: function(data, status, xhr) {
					...
					$('.jp-carousel-overlay').height( $(window).height() + 200);

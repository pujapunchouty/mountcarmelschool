...
function wpzoom_widget_feat_posts() {
	/* Widget settings. */
	$widget_ops = array( 'classname' => 'wpzoom', 'description' => __('Custom WPZOOM widget. Shows posts from one category in a narrow 50% column. Should be paired with another instance of the same widget (if placed in Homepage Widgets sidebar).', 'wpzoom') );
	
	/* Widget control settings. */
	$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'wpzoom-widget-cat' );
	
	/* Create the widget. */
	$this->WP_Widget( 'wpzoom-widget-cat', __('WPZOOM: Featured Posts', 'wpzoom'), $widget_ops, $control_ops );
}
...
function widget( $args, $instance ) {
	...
	<div class="widget wpzoomWidget column<?php if ($wpzoomW == 2) {echo " column-last"; $wpzoomW = 0;} ?>">
...
function wpzoom_register_fposts_widget() {
	register_widget('wpzoom_widget_feat_posts');
}
add_action('widgets_init', 'wpzoom_register_fposts_widget');
?>
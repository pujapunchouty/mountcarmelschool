...
function widget( $args, $instance ) {
		...
		<ul class="wpzoomSocial">
			...
			foreach ($socialProfiles as $item)
			{
				...
				if ($instance[$id] && $instance[$id] != '') {
					...
					<img src="'. get_bloginfo('template_directory') .'/images/icons/'.strtolower($item).'.png" alt="'.$instance[$iditem]. '" /> <span>'.$instance[$iditem]. '</span></a></li>'; 
				}
			} 
			?>
  		</ul>
    ...
    ?>
 		...
 		<p>
			...
			<input type="text" size="35" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>"  />
		</p>
		<br/>
		<?php
		...
		foreach ($socialProfiles as $item)
		{
			...
			?>
 	 		<p>
				<img style="float: left; margin:5px 8px 0 0;" src="<?php echo bloginfo('template_directory') ?>/images/icons/<?php echo $id; ?>.png" />
 				<label for="<?php echo $this->get_field_id( $id ); ?>"><strong><?php echo $item; ?> URL</strong></label> 
				<input type="text" id="<?php echo $this->get_field_id( $id ); ?>" name="<?php echo $this->get_field_name( $id ); ?>" value="<?php echo $instance[$id]; ?>" size="30"/> 

			</p> 
			<p style="margin-left:31px;"> 

 				<label for="<?php echo $this->get_field_id( $idtitle ); ?>">Label</label>
				<input type="text"  id="<?php echo $this->get_field_id( $idtitle ); ?>" name="<?php echo $this->get_field_name( $idtitle ); ?>" value="<?php echo $instance[$idtitle]; ?>" size="30" /><br/>
			</p> 
